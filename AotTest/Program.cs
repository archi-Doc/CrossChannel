...
internal static class Program
{
    ...
    private static async Task<int> Main()
    {
        ...
        Console.WriteLine(failures == 0 ? "All AOT checks passed." : $"{failures} AOT check(s) FAILED.");
    ...
    private static void DependencyInjection()
    {
        ...
        {
            var provider = new ServiceCollection().AddCrossChannel(useRadioClass).BuildServiceProvider();
    ...
    private static void GhostCopyRoundTrip()
    {
        ...
        var from = new CopyTarget { Value = 7 };
        from.Prepare();
        var to = new CopyTarget();
        GhostCopy.Copy(ref from, ref to);
        Check(from.Compare(to), "Copy including init-only and private readonly fields");
    }
}
