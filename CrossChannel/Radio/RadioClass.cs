...
public class RadioClass
{
    ...
    public bool TryGetChannel<TService, TKey>(TKey key, [MaybeNullWhen(false)] out Channel<TService> channel)
        where TService : class, IRadioService
        where TKey : notnull
        => RadioHelper.TryGetChannel(this.twoTypeToMap, key, out channel);
    ...
    /// <returns><c>true</c> if the channel for the specified service type and key is found; otherwise, <c>false</c>.</returns>
    /// <exception cref="InvalidOperationException">Thrown when the service type is not registered.</exception>
    public bool TryGetChannel<TKey>(Type serviceType, TKey key, [MaybeNullWhen(false)] out Channel channel)
        where TKey : notnull
        => RadioHelper.TryGetChannel(this.twoTypeToMap, serviceType, key, out channel);
    ...
    public Channel<TService>.Link Open<TService, TKey>(TService instance, TKey key, bool weakReference = false)
        ...
        where TKey : notnull
    {
        return RadioHelper.GetOrAddChannel(this.twoTypeToMap, instance, key).Open(instance, weakReference);
    }
    ...
    public TService Send<TService, TKey>(TKey key)
        ...
        where TKey : notnull
    {
        if (RadioHelper.TryGetChannel<TService, TKey>(this.twoTypeToMap, key, out var channel))
        {
